...
export default function IntelMap({ articles }) {
  ...
  const fetchLive = useCallback(async (force = false) => {
    ...
    try {
      const [acled, firms] = await Promise.allSettled([
        ...
        fetchFIRMSGlobal(import.meta.env.VITE_FIRMS_KEY || ''),
      ])
  ...
  const stats = React.useMemo(() => ({
    ...
    gpsjam:      (satData?.gpsjam?.length||0) + ((liveAlerts||[]).filter(a=>a.type==='gps_jam').length),
    darkfleet:   satData?.darkfleet?.length||0,
    sar:         satData?.sarRadar?.length||0,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--void)' }}>
      ...
      <div style={{ flexShrink: 0, padding: '6px 12px', borderBottom: '1px solid var(--border)', background: 'var(--void)', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        ...
        {(mapMode === 'environment' ? [
          ...
          { k:'darkfleet',   label:`Dark Fleet (${stats.darkfleet||0})`, color:'#c084fc' },
          { k:'sar',         label:`SAR Radar (${stats.sar||0})`,       color:'#38bdf8' },
          ...
          { k:'bgp',         label:`Internet/BGP (${stats.bgp||0})`,        color:'#ff6600' },
