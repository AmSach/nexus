  ...
  try {
    ...
    if (id === 'intelx') {
      ...
      const r = await fetch('https://free.intelx.io/intelligent/search', {
        method:'POST',
        headers: { 'x-key': keys?.intelx_key || '', 'Content-Type':'application/json' },
    ...
    if (id === 'opensky_auth') {
      ...
      return { status: civil>0?'ok':'warn', detail: civil+' civil aircraft · '+mil+' military · OpenSky key: OpenSky authenticated', count:civil+mil, ms:ms() }
    }
    ...
    if (['adsb_global','opensky_live','airplaneslive','kystdatahuset','digitraffic','barentswatch','vesselfinder','aprs_ships','fleet_live'].includes(id)) {
      ...
      const DETAILS = {
        ...
        opensky_live:  `${(d.aircraft||[]).filter(a=>a.zone==='OpenSky Global').length} aircraft via OpenSky global query (key: OpenSky)`,
