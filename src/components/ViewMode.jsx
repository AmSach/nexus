...
import { useACPL, setClusterFeatsFn } from '../hooks/useACPL'
import { useEmbeddings, CLUSTER_LABELS } from '../hooks/useEmbeddings'
...
export default function ViewMode({ articles = [] }) {
  ...
  // markets and keys from already-imported hooks
  const { markets: sbMarkets } = useSupabaseData()
  const _keys = useStore(s => s.keys)
  ...
  const briefing = useIntelBriefing({
    ...
    articles,
    markets: sbMarkets || [],
    worldVec: null,      // wired from VOX when available
    convergence,
    clusteredArticles,
  ...
  // Train embeddings on articles whenever they change
  useEffect(() => {
    if (!articles?.length || !emb.train) return
    const texts = articles.map(a => a.title).filter(Boolean)
    emb.train(texts)
  }, [articles?.length]) // eslint-disable-line

  // Also train on signal names
  useEffect(() => {
    if (!allPts?.length || !emb.train) return
    const texts = allPts.map(s => s.name).filter(Boolean).slice(0, 500)
    emb.train(texts)
  }, [allPts?.length]) // eslint-disable-line

  // Wire embedding cluster features into ACPL state
  useEffect(() => {
    if (!emb.ready) return
    setClusterFeatsFn((text) => {
      const feats = emb.clusterize(text)
      return feats ? Array.from(feats) : null
    })
  }, [emb.ready]) // eslint-disable-line

  // Wire embedding world vector delta into VOX
  useEffect(() => {
    if (!emb.ready) return
  ...
  const statItems = [
    ...
    { icon:'🔵', label:'EMB',       val:emb.docCount,   clr:'#60a5fa' },
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'var(--void)', overflow:'hidden', fontSize:'12px' }}>
      ...
      <div style={{ flexShrink:0, height:'36px', padding:'0 12px', borderBottom:'1px solid var(--border)',
        ...
        <button onClick={briefing.generate} disabled={briefing.loading || !briefing.groqKey}
          style={{ padding:'2px 8px', borderRadius:'3px', flexShrink:0, fontSize:'9px', fontFamily:'JetBrains Mono,monospace', cursor:'pointer',
            border:`1px solid ${briefing.loading?'#a78bfa':'var(--border)'}`,
            background:briefing.loading?'rgba(167,139,250,0.12)':'rgba(255,255,255,0.04)',
            color:briefing.loading?'#a78bfa':'var(--t4)' }}>
          {briefing.loading ? '🧠 BRIEFING…' : '🧠 BRIEF'}
        </button>
        <Clock />
      </div>
