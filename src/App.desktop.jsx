// NEXUS v4 Desktop — All features, full 12 API routes
  ...
  render() {
    if (this.state.err) return (
      <div style={{ padding:'20px', color:'var(--t3)', fontSize:'12px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:'10px' }}>
        <div>⚠ {this.state.err?.message?.slice(0,80)}</div>
        ...
        <main style={{ flex:1, minWidth:0, overflow:'hidden', display:'flex', flexDirection:'column' }}>
          <Suspense fallback={<div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%', color:'var(--t4)' }}>Loading…</div>}>
            <div style={{ display: tab==='feed' ? 'flex' : 'none', height:'100%', overflow:'hidden', minWidth:0 }}>
              <div style={{ flex:1, minWidth:0, overflow:'hidden' }}><ErrBound><LiveFeed articles={articles} loading={loading} /></ErrBound></div>
              <ErrBound><LiveFeedSidebar articles={articles} /></ErrBound>
            </div>
            <div style={{ display: tab==='situations' ? 'flex' : 'none', height:'100%', overflow:'hidden' }}>
              <div style={{ flex:1, minWidth:0, overflow:'hidden' }}><ErrBound><Situations articles={articles} /></ErrBound></div>
              <div style={{ width:'320px', flexShrink:0, borderLeft:'1px solid var(--border)', overflow:'hidden' }}><ErrBound><CIIDashboard articles={articles} /></ErrBound></div>
            </div>
            ...
            <div style={{ display: tab==='vox' ? 'flex' : 'none', height:'100%', overflow:'hidden', minWidth:0 }}><ErrBound><NEXUSPredict /></ErrBound></div>
          </Suspense>
        </main>
      </div>
    </div>
