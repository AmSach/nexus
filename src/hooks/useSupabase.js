...
export function useSupabaseData() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [sigRes, artRes, mktRes, calRes, logRes] = await Promise.allSettled([
      from('signals')
        .select('id,type,severity,lat,lng,name,desc,url,source,meta,fetched_at,event_date,acpl_action,acpl_ce,acpl_risk_w')
  ...
  const signalPoints = useMemo(() => signals.map(s => ({
    ...
    name: s.name, desc: s.description || s.desc, url: s.url,
    source: s.source,
