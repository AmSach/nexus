...
// DQN: 20 → 32 → 16 → 4 (8 base + 12 cluster dims)
function initDQN() {
  return {
    W1: heInit(32, 20), b1: mat(32, 1),
...
// ── Feature extraction ────────────────────────────────────────────────────────
// Cluster features injected from useEmbeddings (12-dim)
let _getClusterFeats = null
export function setClusterFeatsFn(fn) { _getClusterFeats = fn }

function signalFeatures(signal) {
  // 8 base dims + 12 cluster dims = 20-dim state vector for DQN
  ...
  const base = [
    ...
    signal.lat ? Math.abs(signal.lat) / 90 : 0.5,
    (signal.meta?.fatalities || 0) > 0 ? 1 : 0,
    ['conflict','milaircraft','warship','notam'].includes(signal.type) ? 1 : 0,
  ]
  // Add cluster probability features if embeddings available
  const text = signal.name || signal.title || signal.desc || ''
  const clusterFeats = _getClusterFeats ? _getClusterFeats(text) : null
  if (clusterFeats && clusterFeats.length === 12) {
    return vecFrom([...base, ...clusterFeats])
  }
  return vecFrom([...base, ...new Array(12).fill(1/12)])  // uniform prior if not ready
}
...
// ── Supabase weight sync ──────────────────────────────────────────────────────
const _SB_URL  = import.meta.env.VITE_SUPABASE_URL  || ''
const _SB_ANON = import.meta.env.VITE_SUPABASE_ANON_KEY || ''

async function loadCEFromSupabase() {
  if (!_SB_URL || !_SB_ANON) return null
  try {
    const r = await fetch(`${_SB_URL}/rest/v1/acpl_ce_weights?id=eq.1&select=weights`, {
    ...
    if (!w) return null
    // Convert number[] arrays back to mat objects with Float32Array
    const toMat = (obj) => ({ data: new Float32Array(obj.data), rows: obj.rows, cols: obj.cols })
    return {
      W1: toMat(w.W1), b1: toMat(w.b1), W2: toMat(w.W2), b2: toMat(w.b2),
      W3: toMat(w.W3), b3: toMat(w.b3),
      opt: { W1: adamState(heInit(16,6)), b1: adamState(mat(16,1)), W2: adamState(heInit(8,16)), b2: adamState(mat(8,1)), W3: adamState(heInit(1,8)), b3: adamState(mat(1,1)) }
    }
  } catch { return null }
}

async function pushCEToSupabase(ce) {
  if (!_SB_URL || !_SB_ANON) return
  try {
    const toObj = (m) => ({ data: Array.from(m.data), rows: m.rows, cols: m.cols })
    await fetch(`${_SB_URL}/rest/v1/acpl_ce_weights`, {
      method: 'POST',
      headers: { apikey: _SB_ANON, Authorization: `Bearer ${_SB_ANON}`, 'Content-Type': 'application/json', Prefer: 'resolution=merge-duplicates' },
      body: JSON.stringify({ id: 1, weights: { W1: toObj(ce.W1), b1: toObj(ce.b1), W2: toObj(ce.W2), b2: toObj(ce.b2), W3: toObj(ce.W3), b3: toObj(ce.b3) }, updated_at: new Date().toISOString() }),
      signal: AbortSignal.timeout(5000),
    })
  } catch {}
}
...
export function useACPL({ signals = [], enabled = true } = {}) {
  ...
  // Sync CE weights from Supabase on mount (server may have newer weights from replay)
  useEffect(() => {
    loadCEFromSupabase().then(w => { if (w) { setCe(w); ceRef.current = w } }).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    // Push CE weights to Supabase so server acpl-engine stays in sync
    pushCEToSupabase(ce).catch(() => {})
  }, [step])
  ...
  // Count total network parameters for stats display
  const paramCount = useMemo(() => {
    const ce_ = ceRef.current
    if (!ce_) return 0
    return [ce_.W1, ce_.b1, ce_.W2, ce_.b2, ce_.W3, ce_.b3,
            dqnRef.current?.W1, dqnRef.current?.b1, dqnRef.current?.W2,
            dqnRef.current?.b2, dqnRef.current?.W3, dqnRef.current?.b3]
      .reduce((s, m) => s + (m?.data?.length || 0), 0)
  }, [step])

  return { processedSignals, selectAction, recordOutcome, stats, delayedConsequenceCost,
           replaySize: replay.length, step, paramCount,
           // backward compat alias
           qTableSize: paramCount }
}
