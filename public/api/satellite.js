...
export default async function handler(req, res) {
  ...
  await Promise.race([primaryDeadline, Promise.allSettled([
    ...
    (async () => {
      ...
      const r2 = await get(`https://firms.modaps.eosdis.nasa.gov/api/country/csv/${FIRMS_KEY}/VIIRS_SNPP_NRT/world/1`, 8000)
      if (!r2) return
      try {
    ...
    (async () => {
      ...
      await Promise.allSettled(globalZones.map(async ([minLng,maxLng,minLat,maxLat,label])=>{
        const url = `https://firms.modaps.eosdis.nasa.gov/api/area/csv/${FIRMS_KEY}/VIIRS_SNPP_NRT/${minLng},${minLat},${maxLng},${maxLat}/1`
        const r = await get(url, 6000)
        if (!r) return
        const csv = await r.text()
        if (!csv?.includes('latitude')) return
  ...
  try {
    ...
    await Promise.allSettled(viirs_conflicts.map(async z => {
      const today = new Date().toISOString().slice(0,10)
      const url = `https://firms.modaps.eosdis.nasa.gov/api/area/csv/${FIRMS_KEY}/VIIRS_SNPP_NRT/${z.lng-2},${z.lat-2},${z.lng+2},${z.lat+2}/1`
